    ...
    public class ApiController : Controller
    {
        ...
        public List<StudentFull> GetClassAllStudents(string classId)
        {
            var res = from s in db.Student
                      ...
                      from m in db.Major
                      where
s.UserId == u.Id && s.ClassId == c.Id && c.MajorId == m.Id
                      select new StudentFull()
                      {
                          StudentId = s.UserId,
                          Name = u.Name,
                          Age = u.Age,
        ...
        public void AddCourseToClass(string classId, string courseId)
        {
            ...
            var studentFulls = from s in db.Student
                      from u in db.User
                      from c in db.Class
                      from m in db.Major
                      where
s.UserId == u.Id && s.ClassId == c.Id && c.MajorId == m.Id
            ...
            var stus = (from s in studentFulls where s.ClassId == classId select s).ToList();
...
s.UserId == u.Id && s.ClassId == c.Id && c.MajorId == m.Id
        ...
        [HttpPost("teacherP")]
        public void AddTeacherP(TeacherFull teacher)
        {
            db.Database.ExecuteSqlInterpolated($"exec NewTeacher {teacher.TeacherId} {teacher.Name} {teacher.Age} {teacher.Gender} {teacher.Title} {teacher.Telephone}");
        }
...
s.UserId == u.Id && s.ClassId == c.Id && c.MajorId == m.Id
            ...
            {
                var sscs = (from s in studentFulls where s.StudentId == stu.StudentId select s).Single();
                stu.Name = sscs.Name;
                stu.Age = sscs.Age;
...
s.UserId == u.Id && s.ClassId == c.Id && c.MajorId == m.Id
            ...
            var res = from s in studentFulls
